...
function SplitScreen() {
  ...
  const addPerson = (name: string, color: string, pressed: boolean) => {
    setPersonList(prevList => [...prevList, {
      name: name,
      color: color,
      pressed: pressed,
      deletable: true,
  ...
  const colors = ['#FFA68B', '#948BFF', '#BBEEAE', '#FFDE8B', '#8BE3FF', '#EEAEE7', '#FB8080'];
  ...
  return (
    <View style={styles.container}>
      <View style={[styles.topContainer, { backgroundColor: '#F64610' }]}>
          <Image style={[styles.img]} source={require('../assets/splitt-logo.png')} alt='Logo'/>
      </View>
      <View style={[styles.bottomContainer, { backgroundColor: '#ffff' }]}>
        ...
        <View style={styles.personButtons}>
          {personList.map((person, index) => (
            <TouchableOpacity key={index} style={[styles.squareAvatarFrame, {backgroundColor: person.color}]}>
              <Text>{
                person.deletable ?
          ...
          <TouchableOpacity style={styles.plusButton} onPress={() => setAddPersonModalOpen(true)}>
        ...
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            ...
            <TouchableOpacity style={styles.closeButton} onPress={() => setAddPersonModalOpen(false)}>
